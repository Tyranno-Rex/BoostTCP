...
class MemoryPool {
private:
	std::queue<std::array<char, 1500>> memory_blocks;
	std::mutex pool_mutex;
	std::condition_variable pool_cv;
	size_t pool_size;
...
public:
	...
	MemoryPool(size_t PoolSize) : pool_size(PoolSize) {
		for (size_t i = 0; i < pool_size; i++) {
			memory_blocks.push(std::array<char, 1500>());
		}
	}
	...
	// 메모리 블록을 반환
	void release(std::array<char, 1500>& block) {
		std::lock_guard<std::mutex> lock(pool_mutex);
		memset(block.data(), 0, block.size());
		memory_blocks.push(block);  // 반환된 메모리 블록을 큐에 다시 넣기
		pool_cv.notify_one();     // 대기 중인 스레드에게 알림
	...
	void clear() {
		...
		std::queue<std::array<char, 1500>> empty_queue;
		std::swap(memory_blocks, empty_queue);
	}
};